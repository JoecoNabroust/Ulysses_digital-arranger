  ...
  return (
    <section className="rounded-2xl border border-charcoal/10 bg-white p-5">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 className="font-ui text-sm font-semibold uppercase tracking-wide text-charcoal/70">Character Hub</h3>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        {characters.map((character) => (
          <article key={character.name} className="rounded-xl border border-charcoal/10 bg-parchment p-4">
            ...
            <p className="mt-2 font-ui text-sm text-charcoal/80">{character.relationships.join(" | ")}</p>
            <p className="mt-3 font-ui text-xs text-charcoal/70">Episodes: {character.episodes.join(", ")}</p>
          </article>
        ))}
      </div>
    </section>
