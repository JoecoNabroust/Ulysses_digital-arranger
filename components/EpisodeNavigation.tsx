...
export function EpisodeNavigation({ episodes, activeEpisodeId, onSelectEpisode }: EpisodeNavigationProps) {
  return (
    <nav className="sticky top-6 max-h-[calc(100vh-3rem)] overflow-y-auto rounded-2xl border border-charcoal/10 bg-white p-3">
      <h2 className="mb-3 px-2 font-ui text-xs uppercase tracking-wide text-charcoal/60">Episodes</h2>
          ...
          return (
            <li key={episode.id}>
              <button
                ...
                className={`w-full rounded-lg px-3 py-2 text-left font-ui text-sm transition ${
                  isActive
